...
eBookCore.plugins.zoomview = {
	...
	ratio			: 0,					// 최초 배율값( % )
	origSize	: {	width :0,	// 원본 면적 크기( 확대 축소율 기준으로 사용 )
								height:0 },

	close			: function(){},								// 확대축소 종료
...
/** 확대축소 종료 */
eBookCore.plugins.zoomview.close = function(){
	// 등록 이벤트 제거
	$(window).off(".zoomview");
	// 줌 영역 객체 제거
	$(".zoomview").detach();
...
eBookCore.plugins.zoomview.run = function(_imgEl){
	...
	// 줌 영역 생성
	var zoomEl = $("<div class='zoomview'/>")
								.html("<div class='blind' /><img class='loading' src='" + eBookCore.path.pageview + "loading_zoom.gif' /><span class='zoomviewRatio'>0%</span>")
								.on("mousewheel", function(e)		{ eBookCore.plugins.zoomview.setInOut(e.deltaY, 0.1); })
								.on("pinching"	, function(e,i)	{ eBookCore.plugins.zoomview.setInOut(i.distance, Math.pow(Math.abs(i.distance),1.1)*0.001); })
	...
	$("#zoomviewClose").on(eBookCore.eventType.keyclick, function(e){ eBookCore.eventType.isExcute(e) && eBookCore.plugins.zoomview.close(); });
	$("#zoomviewIn"		).on(eBookCore.eventType.keyclick, function(e){ eBookCore.eventType.isExcute(e) && eBookCore.plugins.zoomview.setInOut( 1, 0.2); });
	$("#zoomviewOut"	).on(eBookCore.eventType.keyclick, function(e){ eBookCore.eventType.isExcute(e) && eBookCore.plugins.zoomview.setInOut(-1, 0.2); });
	...
	$(".zoomview .zoomviewBtn").animate({opacity:1},1000).doTimeout(2000, function(){ this.stop().animate({opacity:0.2},1000); });
	
	// 확대율 텍스트 보였다가 사라짐
	$(".zoomviewRatio").animate({opacity:1}, 1000, function(){
...
eBookCore.plugins.zoomview.setInOut = function(_delta, _ratio){
	...
	if(0>_delta && _wndSize.width>imgEl.width() && _wndSize.height>imgEl.height()){ return eBookCore.plugins.zoomview.close(); }
	
	var _pagePos		= imgEl.offset();
	var _wndCenter	= {	x	: _wndSize.width	/2,
											y	: _wndSize.height	/2 };
	var _centerRatio= {	x	: (_pagePos.left - _wndCenter.x) / imgEl.width(),
	...
	imgEl.css({left:_wndCenter.x + imgEl.width() *_centerRatio.x,
